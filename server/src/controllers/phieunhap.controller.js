...
const findDuplicateMatHang = (chiTiets) => {
  const seen = new Set();
  const duplicates = new Set();

  for (const ct of chiTiets || []) {
    if (!ct || !ct.MaMatHang) continue;
    const maMatHang = String(ct.MaMatHang);
    if (seen.has(maMatHang)) duplicates.add(maMatHang);
    seen.add(maMatHang);
  }
...
const create = async (req, res) => {
  ...
  try {
    ...
    const duplicateMatHangs = findDuplicateMatHang(chiTiets);
    if (duplicateMatHangs.length) {
      await t.rollback();
      return res.status(400).json({
        status: 'error',
        message: 'Mỗi mặt hàng chỉ được chọn một lần trong phiếu nhập.',
      });
    }
