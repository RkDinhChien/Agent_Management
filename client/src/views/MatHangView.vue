    ...
    <div class="ctx-card">
      ...
      <div class="ctx-top">
        ...
        <div class="ctx-actions">
          ...
          />
          <button class="btn-csv" @click="triggerProductImport"><Upload :size="13"/> Import</button>
...
import {
  Search, Plus, Download, X, Eye, Trash2, PackageOpen, Upload,
...
const productImportInputRef = ref(null);
...
const findDvtId = (value) => {
  const raw = String(value ?? '').trim();
  if (!raw) return dvtList.value[0]?.MaDVT ?? null;
  const byId = dvtList.value.find((d) => String(d.MaDVT) === raw);
  if (byId) return byId.MaDVT;
  const normalized = normalizeText(raw);
  return dvtList.value.find((d) => normalizeText(d.TenDVT) === normalized)?.MaDVT ?? null;
};
...
const handleProductImport = async (event) => {
  ...
  try {
    ...
    if (!rows.length) {
      showToast('File import không có dữ liệu.', 'danger');
      return;
    }

    let success = 0;
    const failures = [];
    const existingNames = new Set(products.value.map((p) => normalizeText(p.name)));
    const importedNames = new Set();

    for (const [index, row] of rows.entries()) {
      const rowNumber = index + 2;
      const name = String(pickImportValue(row, ['tenmathang', 'mathang', 'tensanpham', 'sanpham', 'ten', 'name'])).trim();
      const dvtValue = pickImportValue(row, ['madvt', 'dvt', 'donvitinh']);
      const dvtId = findDvtId(dvtValue);
      const normalizedName = normalizeText(name);

      if (!name) {
        failures.push(`Dòng ${rowNumber}: thiếu tên mặt hàng.`);
        continue;
      }
      if (!dvtId) {
        failures.push(`Dòng ${rowNumber}: ĐVT không tồn tại.`);
        continue;
      }
      if (existingNames.has(normalizedName) || importedNames.has(normalizedName)) {
        failures.push(`Dòng ${rowNumber}: mặt hàng "${name}" đã tồn tại.`);
        continue;
      }

      try {
        await api.post('/mat-hang', {
          TenMatHang: name,
          MaDVT: dvtId,
        });
        ...
        failures.push(`Dòng ${rowNumber}: ${parseError(err, 'thêm mặt hàng')}`);
      }
    }

    await loadProducts();
    const suffix = failures.length ? ` Lỗi ${failures.length} dòng: ${failures.slice(0, 3).join(' | ')}` : '';
    showToast(`Import thành công ${success}/${rows.length} mặt hàng.${suffix}`, success ? 'success' : 'danger');
  } catch (err) {
    showToast(`Không đọc được file import: ${err.message}`, 'danger');
  }
};
