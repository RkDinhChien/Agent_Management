      ...
      <div class="ctx-top">
        ...
        <div class="ctx-actions">
          <input
            ...
            accept=".xlsx,.xls,.csv"
            class="file-hidden"
            @change="handleAgentImport"
          />
          <button class="btn-csv" @click="triggerAgentImport"><Upload :size="13"/> Import</button>
...
const agentImportInputRef = ref(null);
...
const pickImportValue = (row, keys) => {
  for (const [header, value] of Object.entries(row)) {
    if (keys.includes(normalizeHeader(header)) && value !== undefined && value !== null && String(value).trim() !== '') {
      return value;
    }
  }
  return '';
...
const findLoaiId = (value) => {
  const raw = String(value ?? '').trim();
  if (!raw) return null;
  const byId = loaiOptions.value.find((l) => String(l.id) === raw);
  if (byId) return byId.id;
  const normalized = normalizeText(raw);
  return loaiOptions.value.find((l) => normalizeText(l.ten) === normalized)?.id ?? null;
};

const findQuanId = (value) => {
  const raw = String(value ?? '').trim();
  if (!raw) return null;
  const byId = quanOptions.value.find((q) => String(q.id) === raw);
  if (byId) return byId.id;
  const normalized = normalizeText(raw);
  return quanOptions.value.find((q) => normalizeText(q.ten) === normalized)?.id ?? null;
};
...
const triggerAgentImport = () => {
  ...
  agentImportInputRef.value?.click();
};
...
const handleAgentImport = async (event) => {
  ...
  try {
    ...
    if (!rows.length) {
      showNotification('error', 'File import không có dữ liệu.');
      return;
    }
    ...
    for (const [index, row] of rows.entries()) {
      ...
      const tenDaiLy = String(pickImportValue(row, ['tendaily', 'daily', 'ten', 'name'])).trim();
      const maLoaiDaiLy = findLoaiId(pickImportValue(row, ['maloaidaily', 'loaidaily', 'loai']));
      const maQuan = findQuanId(pickImportValue(row, ['maquan', 'quan', 'tenquan']));
      const sdt = String(pickImportValue(row, ['sdt', 'sodienthoai', 'phone', 'dienthoai'])).trim();
      const email = String(pickImportValue(row, ['email'])).trim();
      const diaChi = String(pickImportValue(row, ['diachi', 'address'])).trim();
      const ngayTiepNhan = excelDateToISO(pickImportValue(row, ['ngaytiepnhan', 'ngay', 'date']));

      if (!tenDaiLy || !maLoaiDaiLy || !maQuan) {
        failures.push(`Dòng ${rowNumber}: thiếu tên đại lý, loại hoặc quận.`);
        continue;
      }
      if (sdt && !/^0[3-9]\d{8}$/.test(sdt)) {
        failures.push(`Dòng ${rowNumber}: SĐT không hợp lệ.`);
        continue;
      }
      if (email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        failures.push(`Dòng ${rowNumber}: email không hợp lệ.`);
        continue;
      }

      try {
        await api.post('/dai-ly', {
          TenDaiLy: tenDaiLy,
        ...
        failures.push(`Dòng ${rowNumber}: ${parseError(err, 'Không thể thêm đại lý')}`);
      }
    }

    await loadAgents();
    const suffix = failures.length ? ` Lỗi ${failures.length} dòng: ${failures.slice(0, 3).join(' | ')}` : '';
    showNotification(success ? 'success' : 'error', `Import thành công ${success}/${rows.length} đại lý.${suffix}`, failures.length ? 12000 : 6000);
  } catch (err) {
    showNotification('error', `Không đọc được file import: ${err.message}`);
  }
};
